...
public class AnalogyExaminationActivity extends Activity {
	...
	private List<Question> randomDb(int a) {
		List<Question> list = new ArrayList<Question>();
		/*int size;
		try {
			size= (int) dbManager.selector(Question.class).count();
			Log.e("bum","size"+size);
		} catch (DbException e) {
			e.printStackTrace();
		}
		...
		for (int k=1;k<20;k++){
			...
			try {
				...
				question=dbManager.findById(Question.class,k);
	...
	public void uploadExamination(int errortopicNum) {
		...
		if(questionInfos.size()>0){
			...
			if(questionInfos.size()==dataItems.size()){
				...
				//部分选中
				for (int i = 0; i < dataItems.size(); i++) {
					if(dataItems.get(i).getYourAnswer()==null){
						errortopicNums1+=1;
						//保存数据
						SaveQuestionInfo questionInfo=new SaveQuestionInfo();
						questionInfo.setQuestionId(dataItems.get(i).getTopicId());
